...
document.addEventListener("DOMContentLoaded", () => {
  ...
  createForm.addEventListener("submit", (e) => {
    ...
    const isShortPassword = signInPassword.value.length < 8 && confirmPassword.value.length < 8;
    const isPasswordsMismatch = signInPassword.value !== confirmPassword.value;

    if (isShortPassword || isPasswordsMismatch) {
      warningPass.textContent = isShortPassword ? "Password Too Short! (min 8 el)" : "Passwords do not match";
      warningPass.style.opacity = "1";
      warningPass.style.visibility = "visible";
      warningPass.style.transform = "translate(0,0)";
      formRow.style.marginBottom = "35px";
